...
class ModelProvider(str, Enum):
    """지원하는 LLM Provider (기본 + 커스텀)"""
    OPENAI = "openai"
    ANTHROPIC = "anthropic" 
    # OPENROUTER = "openrouter"  # 나중에 구현
    OLLAMA = "ollama"
    CUSTOM = "custom"  # 커스텀 모델 지원 추가
...
def load_custom_models() -> List[ModelInfo]:
    """custom_config.json에서 커스텀 모델 로드"""
...
def validate_api_key(provider: ModelProvider) -> bool:
    ...
    key_map = {
        ...
        ModelProvider.CUSTOM: "CUSTOM_API_KEY"  # 커스텀 API 키
    }
...
def list_available_models() -> List[Dict[str, Any]]:
    ...
    # 커스텀 모델들 (阿里百炼 등 국내 모델)
    all_models.extend(load_custom_models())
...
def load_llm_model(model_name: str, provider: str, temperature: float = 0.0):
    ...
    if provider_enum == ModelProvider.ANTHROPIC:
    ...
    elif provider_enum == ModelProvider.CUSTOM:
        # 커스텀 모델 로드 (OpenAI 호환 API)
        from langchain_openai import ChatOpenAI
        
        # 커스텀 설정 파일에서 API 정보 찾기
        custom_config = get_custom_model_config(model_name)
        if not custom_config:
            raise ValueError(f"Custom model configuration not found: {model_name}")
        ...
        return ChatOpenAI(
            ...
            openai_api_base=custom_config.get("api_base", "https://api.openai.com/v1"),
            temperature=0
        )
...
def get_custom_model_config(model_name: str) -> Optional[Dict[str, Any]]:
    """커스텀 모델 설정 정보 가져오기"""
    config_path = Path(__file__).parent / "custom_config.json"
    ...
    try:
        ...
        for model_data in models_data:
            if model_data.get("model_name") == model_name and model_data.get("provider") == "custom":
                return model_data
